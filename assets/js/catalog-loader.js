document.addEventListener("DOMContentLoaded", () => {
  ...
  const baseApi = "https://bestgift-backend.onrender.com"; // backend sans slash
          ...
          card.innerHTML = `
            <div class="product-card">
              ...
              <div class="detail-box">
                ...
                <button 
                  class="btn btn-sm btn-primary snipcart-add-item mt-2"
                  data-item-id="${p.id || p.title.replace(/\s+/g, '-').toLowerCase()}"
                  data-item-name="${p.title}"
                  data-item-price="${price}"
                  data-item-url="${url}"
                  data-item-description="${p.description || 'Cadeau BestGift'}"
  ...
  // Rafraîchissement au changement des filtres
